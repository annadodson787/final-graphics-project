...
vec3 lighting(vec4 baseColor, vec3 n, vec3 v, vec4 causticColor) {
	vec3 color = vec3(0);
	for(int i=0;i<lt_att[0];i++) {
		color+=lamb(i,baseColor,n,v, causticColor);
	}
	return color;
}
...
void main()
{
	vec4 baseColor = texture(tex, vtx_normalized_uv);
	vec4 waterColor = vec4(0.22f, 0.28f, 0.3f, 1.0f);
	...
	vec3 color = lighting(baseColor+waterColor, norm, vertex_to_eye, causticColor);
