...
void main()
{
	...
	//vec3 color = length(emissiveColor)*vec3(.7, .4, 0) + lighting(baseColor, norm, vertex_to_eye);
	//frag_color=vec4(color.rgb,1.f);
    
    // Implement fog as mix of background water color and visibility
    frag_color = mix(background_color, baseColor, clamp(visibility*4, 0, 1));
    //frag_color = vec4(frag_color.x + .1, frag_color.y + .1, frag_color.z + .1, .9);
    frag_color.w = .75;
}
